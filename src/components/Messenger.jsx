...
import { Send, Search, X, Plus, Loader2 } from 'lucide-react'
...
// The actual chat UI: conversation list + thread. Used both as the body of
// the full-page /messages route and inside the floating chat modal, so it
// stays independent of whatever page-chrome wraps it.
export default function Messenger({ initialConversationId = null, onConversationOpen }) {
  const { user } = useAuth()
  const [searchParams, setSearchParams] = useSearchParams()
  const [conversations, setConversations] = useState([])
  const [activeId, setActiveId] = useState(initialConversationId ?? searchParams.get('c') ?? null)
  ...
  useEffect(() => {
    if (activeId) {
      loadThread(activeId)
      onConversationOpen?.(activeId)
      // Only the full page keeps the conversation in the URL — the
      // floating modal has no route of its own to sync with.
      if (onConversationOpen === undefined) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`messages_${user.id}_${onConversationOpen ? 'floating' : 'page'}`)
    ...
    <div className="chat-shell">
      <aside className="chat-sidebar">
        <div className="chat-sidebar-header">
          <button className="btn btn-sm" onClick={() => { haptic('tap'); setShowPicker((s) => !s) }}>
